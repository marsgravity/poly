...
u64 PowerMod(u64 a, int n, u64 c = 1) {
    for (; n; n >>= 1, a = a * a % mod) if (n & 1) c = c * a % mod;
    return c;
}
...
namespace poly_base {
    ...
    void init(int n = N, bool dont_calc_factorials = true) {
        ...
        if (!dont_calc_factorials)
            for (*finv = *fact = i = 1; i < n; ++i) {
                ...
                finv[i] = (u64) finv[i - 1] * inv[i] % mod;
            }

        t = min(n > 1 ? std::__lg(n - 1) : 0, 21),
    ...
    void DIT(int *a) {
        ...
        for (i = 0; i < l; ++i, len <<= 1)
            for (j = a, o = w2; j != a + n; j += len << 1, ++o)
                for (k = j; k != j + len; ++k) {
                    reduce(R = *k + k[len] - mod);
                    k[len] = u64(*k - k[len] + mod) * *o % mod;
                    *k = R;
                }
...
namespace poly {
    ...
    void inv(int deg, pvec a, pvec b) {
        ...
        memset(b + 1, 0, i = 8 << std::__lg(deg - 1));
        memset(B1, 0, i);
        *B1 = *a;
    ...
    void div_mod(int A, int B, pvec a, pvec b, pvec q, pvec r) {
        ...
        int Q = A - B, i, l_ = Q ? std::__lg(Q) + 1 : 0;
        NTT_init(l_);
        for (i = 0; i <= Q && i <= B; ++i) B4[i] = b[B - i];
        memset(B4 + i, 0, (n - i) << 2);
        inv(i = Q + 1, B4, B5);

        std::reverse_copy(a + B, a + (A + 1), B4);
        NTT_init(++l_);
        memset(B4 + i, 0, (n - i) << 2);
        memset(B5 + i, 0, (n - i) << 2);
        mul(2 * Q, B4, B5, q);
    ...
    void mul(std::vector<int> &a, std::vector<int> &b, std::vector<int> &ret) {
        int A = a.size() - 1, B = b.size() - 1;
        if (!(A || B)) {
            ret.emplace_back((u64) a[0] * b[0] % mod);
            return;
        }
        NTT_init(std::__lg(A + B) + 1);
        memcpy(B1, a.data(), (A + 1) << 2);
        memset(B1 + (A + 1), 0, (n - A - 1) << 2);
        memcpy(B2, b.data(), (B + 1) << 2);
        memset(B2 + (B + 1), 0, (n - B - 1) << 2);
        DNTT(B1), DNTT(B2);
    ...
    // Integral
    void intg(int deg, pvec a, pvec b, int constant = 0) {
        for (int i = deg; i; --i)
            b[i] = (u64) a[i - 1] * ::inv[i] % mod;
        *b = constant;
    }
    ...
    void dif_quo(int deg, pvec a, pvec b) {
        assert(*a);
        if (deg <= 1) {
            *b = PowerMod(*a, mod - 2, a[1]);
            return;
        }

        int i, len = std::__lg(deg - 1);
        inv((deg + 1) / 2, a, B4);
        NTT_init(len + 1);
        memset(B4 + (n >> 1), 0, n << 1);
        DIF(B4, B2);
        ...
        DIF(b, B4), DIF(a, B3);
        for (i = 0; i < n; ++i) B3[i] = (u64) B3[i] * B4[i] % mod;
        DIT(B3);
    ...
    inline void ln(int deg, pvec a, pvec b) {
        assert(*a == 1), --deg ? (dif_quo(deg, a, b), intg(deg, b, b)) : void(*b = 0);
    }
    ...
    void exp(int deg, pvec a, pvec b) {
        ...
        memset(b + 2, 0, i = 8 << std::__lg(deg - 1));
        memset(c, 0, i);
        memset(B1, 0, i);
        *c = 1;
        neg(c[1] = b[1]);
        ...
        for (len = 1; 1 << len < deg; ++len) {
            ...
            DIF(c, B2), DIF(b, B3);
            for (i = 0; i < n; ++i) B4[i] = (u64) B3[i] * B2[i] % mod;
            DIT(B4);
            for (i = n >> 1; i < n; ++i) B4[i] = B4[n - i] * iv % mod;

            memset(B4, 0, n << 1), DIF(B4);
            for (i = 0; i < n; ++i) B4[i] = (u64) B4[i] * B2[i] % mod;
            DIT(B4);
            for (i = n >> 1; i < n; ++i) B4[i] = B4[n - i] * (mod - iv) % mod;

            memcpy(B4, c, n << 1), DIF(B4);
            diff(n >> 1, b, B1), DIF(B1, B5);
            for (i = 0; i < n; ++i) B4[i] = (u64) B4[i] * B5[i] % mod;
            DIT(B4);
            for (i = n >> 1; i < n; ++i) {
                reduce(B5[i] = (a[i] + B4[n - i + 1] * (mod - iv) % mod * ::inv[i]) % mod);
            }

            memset(B5, 0, n << 1), DIF(B5);
            for (i = 0; i < n; ++i) B5[i] = (u64) B5[i] * B3[i] % mod;
            DIT(B5);
...
namespace poly_evaluation {
    ...
    void recursion(int id, int L, int R, const std::vector<int> &poly) {
        if (L + 1 == R) {
            tmp_.emplace_back(poly.back());
            return;
        }
        int i, n = poly.size() - 1, M = (L + R) / 2, lp = lc[id], rp = rc[id],
                dl = min(n, g[lp].size() - 1), dr = min(n, g[rp].size() - 1);
        if (L + 2 == R) {
            tmp_.emplace_back((poly[n] + (u64) poly[n - 1] * g[rp].back()) % mod);
            tmp_.emplace_back((poly[n] + (u64) poly[n - 1] * g[lp].back()) % mod);
            return;
        }
        std::vector<int> ly, ry;
        ly.reserve(dl + 1), ry.reserve(dr + 1);
        NTT_init(std::__lg(dl + dr) + 1);
        memcpy(E1, poly.data(), (n + 1) << 2);
        DIF(E1, E2);
        memset(E1, 0, (n + 1) << 2);

        memcpy(E1, g[rp].data(), (dr + 1) << 2);
        DIF(E1, E3);
        memset(E1, 0, (dr + 1) << 2);
        for (i = 0; i < poly::n; ++i) E3[i] = (u64) E3[i] * E2[i] % mod;
        DIT(E3), std::reverse(E3 + 1, E3 + poly::n);
        for (i = n - dl; i <= n; ++i) ly.emplace_back(E3[i] * iv % mod);

        memcpy(E1, g[lp].data(), (dl + 1) << 2);
        DIF(E1, E3);
        memset(E1, 0, (dl + 1) << 2);
        for (i = 0; i < poly::n; ++i) E3[i] = (u64) E3[i] * E2[i] % mod;
        DIT(E3);
        std::reverse(E3 + 1, E3 + poly::n);
        for (i = n - dr; i <= n; ++i) ry.emplace_back(E3[i] * iv % mod);

        recursion(lp, L, M, ly), recursion(rp, M, R, ry);
    }

    std::vector<int> emain(int n, pvec f, const std::vector<int> &pts) {
        int i, id, m = pts.size(), q;
        if (!m) return std::vector<int>();
        if (!n) return std::vector<int>(m, *f);
        for (i = 0; i < m; ++i) g[i].clear(), g[i].emplace_back(1), g[i].emplace_back(neg(q = pts[i]));

        id = solve(0, cnt = m);
        memcpy(Prd_, g[id].data(), (m + 1) << 2);
        poly::inv(n + 1, Prd_, E2);
        memset(Prd_, 0, (m + 1) << 2);
        if (n > 0) memset(E2 + (n + 1), 0, (poly::n - n - 1) << 2);

        std::reverse_copy(f, f + (n + 1), E1);
        poly::mul(2 * n, E1, E2, E3);
        memset(E1, 0, (n + 1) << 2);
        memset(E2, 0, (n + 1) << 2);

        tmp_.clear();
        tmp_.reserve(m);
        recursion(id, 0, m, std::vector<int>(E3 + max(n - m + 1, 0), E3 + (n + 1)));
        return tmp_;
    }
}
...
namespace poly_interpolation {
    ...
    void irecursion(int id, int L, int R) {
        ...
        NTT_init(std::__lg(R - L - 1) + 1);
        memcpy(I1, iresult + L, (M - L) << 2);
        memset(I1 + (M - L), 0, (poly::n - (M - L)) << 2);
        memcpy(I2, iresult + M, (R - M) << 2);
        memset(I2 + (R - M), 0, (poly::n - (R - M)) << 2);
        memcpy(I3, g[lp].data(), (M - L + 1) << 2);
        memset(I3 + (M - L + 1), 0, (poly::n - (M - L + 1)) << 2);
        memcpy(I4, g[rp].data(), (R - M + 1) << 2);
        memset(I4 + (R - M + 1), 0, (poly::n - (R - M + 1)) << 2);
        DIF(I1), DIF(I2), DIF(I3), DIF(I4);
        for (i = 0; i < n; ++i) I1[i] = ((u64) I1[i] * I4[i] + (u64) I2[i] * I3[i]) % mod;
        DIT(I1);
        std::reverse(I1 + 1, I1 + n);
        for (i = 0; i < R - L; ++i) iresult[L + i] = I1[i] * iv % mod;
    }

    void imain(int n, std::pair<int, int> *pts, pvec ret) {
        int i, id, q;
        assert(n > 0);
        for (i = 0; i < n; ++i) g[i].clear(), g[i].emplace_back(1), g[i].emplace_back(neg(q = pts[i].first));

        id = solve(0, cnt = n);
        memcpy(Prd_, g[id].data(), (n + 1) << 2);
        ...
        tmp_.clear(), tmp_.reserve(n);
        recursion(id, 0, n, std::vector<int>(E3, E3 + n));
        for (i = 0; i < n; ++i) ret[i] = PowerMod(tmp_[i], mod - 2, pts[i].second);
        iresult = ret;
        irecursion(id, 0, n);
        std::reverse(ret, ret + n);
    }
}
...
namespace linear_recur {
    ...
    void solve(int Q, int d) {
        int i, df = 1, z = std::__lg(Q);
        bool alive = false;
        assert(d > 0), ld = std::__lg(2 * d - 1) + 1;
...
namespace miscellaneous {
    ...
    void czt(int deg, int len, int z, pvec a, pvec b) {
        int i;
        u64 iz = PowerMod(z, mod - 2);
        if (!len) return;
        if (!deg || !z) return std::fill(b, b + len, *a);
        NTT_init(std::__lg(deg + len - 1) + 1);
        M1[1] = *M1 = M3[1] = *M3 = 1;
...
#undef term
    ...
    void cinv(int deg, pvec a, pvec b) {
        ...
        NTT_init(std::__lg(2 * deg - 1));
        memcpy(M1, a + 1, deg << 2);
        memset(M1 + deg, 0, (n - deg) << 2);
        poly::inv(deg, M1, M2);

        NTT_init(std::__lg(2 * deg - 1) + 1);
